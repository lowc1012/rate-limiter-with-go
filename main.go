...
import (
	...
	rl "github.com/lowc1012/rate-limiter-with-go/rate_limiter"
	"go.uber.org/zap"
)
...
var (
	...
	stateStrings              = map[rl.State]string{
		rl.Allow: "Allow",
		rl.Deny:  "Deny",
	}
)
...
func (h *httpRateLimiterHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	result, err := h.config.Strategy.Run(request.Context(), &rl.Request{
		Key:      key,
	...
	writer.Header().Set(rateLimitingExpiresAt, result.ExpiredAt.Format(time.RFC3339))

	// when the state is Deny, just return a 429 response to the client and stop the request handling flow
	if result.State == rl.Deny {
...
func main() {
	...
	config := &RateLimiterConfig{
		Extractor:   NewHTTPHeadersExtractor("X-Forwarded-For"),
		Strategy:    strategy,
		Expiration:  time.Minute,
		MaxRequests: 10,
	}

	wrappedMux := NewHTTPRateLimiterHandler(mux, config)
