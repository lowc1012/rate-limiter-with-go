...
func TestSlidingWindowStrategy_Run(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			name: "expires and starts again (returns Allow)",
			runs: 100,
			request: &Request{
				Key:      "user",
				Limit:    60,
			...
			lastResult: &Result{
				State:         Allow,
				TotalRequests: 59,
				ExpiredAt:     time.Date(2022, 5, 10, 9, 17, 39, 0, time.UTC),
			},
			advance: time.Second,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			strategy := NewSlidingWindowStrategy(client, func() time.Time {
				return now
			})
